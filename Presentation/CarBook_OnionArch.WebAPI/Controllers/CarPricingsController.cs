...
namespace CarBook_OnionArch.WebAPI.Controllers
{
    ...
    public class CarPricingsController(IMediator mediator) : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<ActionResult> Create(CreateCarPricingCommand command)
        {
            try
            {
        ...
        [HttpPut("{id}")]
        public async Task<ActionResult> Update(int id, UpdateCarPricingCommand command)
        {
            if (id != command.Id)
        ...
        public async Task<ActionResult> Delete(int id)
        {
            try
            {
                var result = await mediator.Send(new RemoveCarPricingCommand(id));
