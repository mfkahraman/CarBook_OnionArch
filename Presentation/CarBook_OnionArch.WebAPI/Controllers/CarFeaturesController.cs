...
namespace CarBook_OnionArch.WebAPI.Controllers
{
    ...
    public class CarFeaturesController(IMediator mediator) : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<ActionResult> Create(CreateCarFeatureCommand command)
        {
            try
            {
        ...
        [HttpPut("{id}")]
        public async Task<ActionResult> Update(int id, UpdateCarFeatureCommand command)
        {
            if (id != command.Id)
        ...
        public async Task<ActionResult> Delete(int id)
        {
            try
            {
                var result = await mediator.Send(new RemoveCarFeatureCommand(id));
